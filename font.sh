...
GENERATE_FONT() {
  rm -f $(echo "$OUTPUTPATH/../index.css")
  for i in "${!SUBSETNAMES[@]}"; do
    for j in "${!FORMATS[@]}"; do
      for k in "${!FILENAMES[@]}"; do
        ...
        REPLACE=$(echo "s/${INPUT//\//\\/}/${OUTPUT//\//\\/}/")
        REPLACE=$(echo "${REPLACE//\.\\\//}")
        REPLACE=$(echo "${REPLACE//\./\\\.}")
        REPLACE=$(echo "${REPLACE//src\\\//\/}")
        REPLACE=$(echo "${REPLACE//service\//assets\/}")
        echo "----------------\n$INPUT -> $OUTPUT\n----------------\n"
        ...
        sed $(echo $REPLACE) $(echo "${INPUTPATH//\.\//}/${FILENAMES[k]}.css") | sed s/file/record/ >> $(echo "$OUTPUTPATH/../index.css")
        echo "\n\n" >> $(echo "$OUTPUTPATH/../index.css")
        rm $(echo "$INPUTPATH/${FILENAMES[k]}.css")
      done
    done
  done
}
