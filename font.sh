  ...
  for i in "${!SUBSETNAMES[@]}"; do
    for j in "${!FORMATS[@]}"; do
      for k in "${!FILENAMES[@]}"; do
        ...
        REPLACE="s!${INPUT//\.\/service/service}!${OUTPUT//\.\/public\/assets/\/assets}!"
        echo "----------------\n$INPUT -> $OUTPUT\n----------------\n"
        ...
        mkdir -p "$OUTPUTPATH"
        mv $INPUT $OUTPUT
        cat "${INPUTPATH//\.\//}/${FILENAMES[k]}.css" | sed "$REPLACE" | sed s/file/record/ | sed "$FONT_FAMILY" | sed -e "${FONT_STYLE//\:\ Regular/\:\ normal}" | sed s/\:\ Bold/\:\ bold/ | sed s/\:\ Medium/\:\ 500/ >> "$OUTPUTPATH/../index.css"
...
OUTPUTPATH="./public/assets/fonts/Fira-Sans"
GENERATE_FONT
...
OUTPUTPATH="./public/assets/fonts/Fira-Mono"
GENERATE_FONT